...
- Audio Convert's migration removed an implicitly shared `escapeHtml` helper
  that still served the home grid, feedback marquee, favorites and cleanup
  surfaces. The helper now has an explicit shared module and contract test.
- Homepage lazy cards relied on replaying clicks on hidden legacy category
  items. The direct registry path now opens migrated lazy features reliably in
  every category state.
- Audio Convert disposal could schedule a final process-button hide timer
  after the lifecycle had already been marked disposed. The disposed guard now
  clears and hides the control synchronously.
...
Audio Convert now lives in the lazy `src/features/audio-convert/` boundary.
Its `template.js` owns the existing page markup, `controller.js` owns queue
rendering, sortable-file lifecycle, native drag/drop, FFmpeg gating, progress,
cancellation, output publication and success-dialog state, and
`audio-convert.css` loads only with the feature. The controller now uses
feature-scoped `data-*` hooks, releases queue listeners and conversion timers,
and invalidates stale results on close or dispose. Homepage cards call the lazy
registry directly, so tool opening no longer depends on replaying a click on a
hidden legacy category item. The shared `src/shared/html.js` module restores
explicit HTML/attribute escaping for the app shell after the old nested helper
was removed with the legacy controller.

The Audio Convert core, runtime, shared HTML and lazy-tool contract suites pass.
Fresh-browser checks cover first open, close, reopen, queue reset, visible
controls and console warning/error inspection. The production output emits an
approximately 11.19 kB feature CSS chunk and an independent lazy JavaScript
...
BPM Detect and Audio Clip now share the lazy `src/features/audio-tools/`
boundary while retaining separate lifecycle controllers and feature styles.
BPM owns analysis request identity, dynamic tempo dependencies, native drop
registration, demo playback nodes and AudioContext cleanup. Audio Clip owns
file decoding, high-DPI waveform rendering, pointer-handle drag scopes,
playback, FFmpeg export identity, native drop registration and output-dialog
state. Closing either page releases its listeners and asynchronous resources;
stale work cannot update a later open. The former 2,188-line aggregate feature
stylesheet is split by tool responsibility into `bpm.css` and
`audio-clip.css`, both below the 1,200-line review limit.

The BPM core/runtime suites, Audio Clip core/runtime suites, audio lazy and
lifecycle contract, architecture gate, production build and `git diff
--check` pass. A fresh page load at 1280 by 720 covers both lazy imports,
first open, close, reopen and hidden-state ownership; no console or page error
was observed. The build emits an approximately 43.16 kB shared audio JavaScript
chunk, a 34.87 kB feature CSS chunk and a separately loaded 14.23 kB BPM
...
Color Extractor now lives in the lazy `src/features/color-extractor/` boundary.
Its controller owns image-byte validation, Object URL lifetime, Canvas palette
extraction, palette/detail rendering, browser drag/drop, native WebView drag/drop,
screen-mode selection, clipboard feedback and operation invalidation on close.
The screen-picker overlay is a separate dynamically loaded controller so the
main page never runs its sampling loop or exposes its canvas. The native Rust
window is intentionally reused; a reopen event now reactivates the picker
controller before resetting its sampling loop, preventing the first-close state
from disabling subsequent picks. The legacy `src/color-extractor-core.js`
compatibility path remains available for existing consumers.

The Color Extractor core, runtime and lazy lifecycle contract suites pass.
Fresh-browser checks cover a real PNG upload and palette output, image/screen
mode switching, close/reopen state reset, repeated page entry, hidden screen
picker isolation and console warning/error inspection. The production build
emits an approximately 5.19 kB screen-picker JavaScript chunk and 21.24 kB
feature CSS chunk; main JavaScript is 1,252.00 kB and main CSS is 362.81 kB.
The architecture gate, production build and `git diff --check` pass. The full
