// Adapted from Joshua-Zion's PR #67 (Apache-2.0); V3 input/lifecycle integration.
/**
...
export function createColorWheels(mount, {
  ...
  function applyFromPointer(wheel, event) {
    const { x, y } = pointerToNorm(wheel, event);
  ...
  function flushPointer() {
    ...
    if (active && pending) applyFromPointer(pending.wheel, pending.event);
  }

  function releasePointer(wheel) {
    const id = wheel.pointerId;
    wheel.pointerId = null;
    wheel.mode = null;
    if (id !== null) {
      try { wheel.overlay.releasePointerCapture(id); } catch { /* already released */ }
    }
  }

  function bindPointer(wheel) {
    lifecycle.event(wheel.overlay, 'pointerdown', (event) => {
      if (!active || wheels.some(entry => entry.pointerId !== null)) return;
      ...
      if (r > 1) return;
      // Finish numeric/HEX drafts before capturing the model for a new pick.
      beforeInteraction();
      wheel.overlay.focus({ preventScroll: true });
    ...
    lifecycle.event(wheel.overlay, 'pointermove', (event) => {
      if (wheel.pointerId !== event.pointerId) return;
      event.preventDefault();
      pendingPointer = { wheel, event: { clientX: event.clientX, clientY: event.clientY } };
      if (pointerFrame === null) pointerFrame = requestAnimationFrame(flushPointer);
    });
    const stop = (event) => {
      if (wheel.pointerId !== event.pointerId) return;
      if (event.type === 'pointerup') flushPointer();
      else cancelPendingPointer();
      releasePointer(wheel);
    };
    lifecycle.event(wheel.overlay, 'pointerup', stop);
    lifecycle.event(wheel.overlay, 'pointercancel', stop);
    lifecycle.event(wheel.overlay, 'lostpointercapture', stop);
    lifecycle.event(wheel.overlay, 'keydown', (event) => {
      if (!active) return;
      ...
      const values = getValues(wheel.kind.id);
      if (horizontal) {
        values.h = (values.h + horizontal * 5 + 360) % 360;
      } else if (wheel.kind.id === 'hsv') {
  ...
  return {
    ...
    refreshLabels(getText) {
      for (const wheel of wheels) {
        ...
        const label = getText('label', { space: wheel.kind.label });
        wheel.overlay.setAttribute('aria-label', label);
        wheel.overlay.title = label;
      }
    },
    destroy() {
      close();
